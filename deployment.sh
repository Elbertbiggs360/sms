...
docker build -t enkya.org/${PROJECT_ID}/${REG_ID}:$CIRCLE_SHA1 .

gcloud docker -- push enkya.org/${PROJECT_ID}/${REG_ID}:$CIRCLE_SHA1

kubectl set image deployment/${DEPLYMENT_NAME} ${CONTAINER_NAME}=enkya.org/${PROJECT_ID}/${REG_ID}:$CIRCLE_SHA1
